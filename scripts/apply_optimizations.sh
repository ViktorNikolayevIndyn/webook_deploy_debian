...
optimize_project() {
  ...
  if ask_yes_no "[opt] Update deploy.sh with optimized version?" "y"; then
    if [ -f "$SCRIPT_DIR/deploy.template.sh" ]; then
      ...
      cp "$SCRIPT_DIR/deploy.template.sh" "deploy.sh"
      chmod +x "deploy.sh"
      echo "[opt]   ✓ deploy.sh updated"
    else
      echo "[opt]   ERROR: Template not found: $SCRIPT_DIR/deploy.template.sh"
    fi
  fi
  ...
  if ask_yes_no "[opt] Replace Dockerfile with optimized multi-stage version?" "n"; then
    if [ -f "$SCRIPT_DIR/Dockerfile.optimized" ]; then
      ...
      cp "$SCRIPT_DIR/Dockerfile.optimized" "Dockerfile"
      ...
      echo "[opt]   WARNING: Template not found: $SCRIPT_DIR/Dockerfile.optimized"
    fi
  fi
  
  # 3. Обновить docker-compose.yml
  if ask_yes_no "[opt] Replace docker-compose.yml with optimized version?" "n"; then
    if [ -f "$SCRIPT_DIR/docker-compose.optimized.yml" ]; then
      ...
      cp "$SCRIPT_DIR/docker-compose.optimized.yml" "docker-compose.yml"
      ...
      echo "[opt]   WARNING: Template not found: $SCRIPT_DIR/docker-compose.optimized.yml"
    fi
  fi
  ...
  if [ ! -f ".dockerignore" ]; then
    if ask_yes_no "[opt] Create .dockerignore file?" "y"; then
      if [ -f "$SCRIPT_DIR/.dockerignore.example" ]; then
        echo "[opt]   Creating .dockerignore..."
        cp "$SCRIPT_DIR/.dockerignore.example" ".dockerignore"
