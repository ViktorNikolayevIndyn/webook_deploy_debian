# Git Webhook Deploy for Debian / Proxmox (Docker + Cloudflare‑ready)
...
- Git push → Git provider sends a webhook → `webhook.js` receives it → matches a project in `config/projects.json` → runs that project’s `deploy.sh` with arguments.
...
- 🔁 **Git webhook → deployment** per repo/branch.
- 📦 **Multiple projects** in one config.
- 🐳 **Docker‑friendly**: deploy scripts are intended to run `docker compose build/up` etc.
- ☁️ **Cloudflare Tunnel metadata** per project (subdomain, port, path, tunnel name) – Cloudflare itself is managed separately.
- ⚙️ **Bootstrap scripts**:
  - `env-bootstrap.sh` – base environment (APT tools, Docker, etc.).
  - `enable_ssh.sh` – optional SSH user + sudo + docker group.
  - `init.sh` – interactive wizard for webhook + projects config.
- 🔍 **check_env.sh** – sanity checks for Docker, flags, config and webhook port.
- 📂 **file_structure.sh** – interactive helper to explore and understand the repository structure.
- 🧠 **State files with versions**:
  - `config/env_bootstrap.json`
  - `config/ssh_state.json`
  - `config/projects_state.json`  
  Used by `start.sh` to decide when to re‑run steps after script changes.
...
├─ file_structure.sh         # file structure helper (NEW)
├─ config/
│  ├─ projects.example.json  # example config with placeholders
│  ├─ projects.json          # real config (generated by init.sh)
│  ├─ env_bootstrap.json     # env-bootstrap state (auto)
│  ├─ ssh_state.json         # SSH / user state (auto)
│  └─ projects_state.json    # init.sh state (auto)
└─ scripts/
   ├─ env-bootstrap.sh       # install tools (Docker, curl, git, node, etc.)
   ├─ enable_ssh.sh          # optional SSH user + groups setup
   ├─ init.sh                # configuration wizard (webhook + projects)
   ├─ check_env.sh           # environment & config checks
   └─ deploy.template.sh     # template copied to each project as deploy.sh
```

> Only `projects.example.json` is tracked by git.  
> `projects.json` and the state files are generated on the server.

### 2.1. File Structure Helper

Use `file_structure.sh` to explore and understand the repository structure:

```bash
./file_structure.sh --help       # Show usage information
./file_structure.sh --summary    # Show structure summary (default)
./file_structure.sh --detailed   # Show detailed descriptions
./file_structure.sh --verify     # Verify all expected files exist
./file_structure.sh --config     # Show config directory structure
./file_structure.sh --tree       # Show full directory tree (requires 'tree')
```

This helper provides:
- 📊 Visual representation of the repository structure
- 📝 Descriptions of each file and directory
- ✓ Verification of repository integrity
- 🔍 Config directory inspection
...
2. **Environment bootstrap** – offer to run `scripts/env-bootstrap.sh`.
...
3. **SSH setup** – offer to run `scripts/enable_ssh.sh`.
...
4. **Project & webhook config** – offer to run `scripts/init.sh`.
...
On next `./start.sh`, it will notice the mismatch and offer to re‑run the step (default = **Yes** for changed versions).
...
- `port` – local port for `webhook.js` (default `4000`).
- `path` – HTTP path that will receive the webhook (default `/github`).
- `secret` – optional shared secret for signature verification.
- `cloudflare.enabled` – Boolean flag, normally `true` if you want to expose webhook via Cloudflare.
- `cloudflare.rootDomain` – e.g. `"linkify.cloud"`.
- `cloudflare.subdomain` – e.g. `"webhook"` → full host `webhook.linkify.cloud`.
- `cloudflare.localPort` – port where the webhook server listens (same as `port`).
- `cloudflare.localPath` – path on the local server (usually same as `path`).
- `cloudflare.protocol` – `"http"` or `"https"` for the origin URL inside the tunnel.
- `cloudflare.tunnelName` – logical name of the tunnel you use for this root domain.
...
- `name` – internal name, e.g. `"linkify-dev"`.
- `gitUrl` – repo URL for cloning, e.g.  
...
- `cloudflare` – project‑specific domain mapping:
  ...
  - `localPort` – internal app port (e.g. `3000`).
  - `localPath` – usually `"/"`.
  - `protocol` – `"http"` or `"https"`.
  - `tunnelName` – which Cloudflare tunnel should serve this root domain.
...
## 5. deploy.template.sh → per‑project deploy.sh
...
7. Spawns the project’s `deployScript` with `deployArgs`.
...
8. Push to the configured branches → deployment is triggered automatically.
